...
export interface InitNuveiSimplyConnectAction {
  type: 'nuveiSimplyConnect';
  sessionToken: string;
}

export type PaymentAction = RedirectAction | ThreeDS2Action | InitNuveiSimplyConnectAction;
...
export interface NuveiPaymentRequestPayload {
  orderNumber: string;
  amountPlanned: {
    currencyCode: string;
    centAmount: number;
  };
  paymentMethod: Pick<NuveiData, 'type'>;
}

export type PaymentRequestPayload =
  | SchemePaymentRequestPayload
  | KlarnaPaymentRequestPayload
  | NuveiPaymentRequestPayload;
...
export interface NuveiData {
  type: 'nuvei';
}

export type PaymentData = SchemeData | KlarnaData | NuveiData;
...
export interface PaymentProvider {
  ...
  makeNuveiPayment: (data: Omit<NuveiPaymentRequestPayload, 'paymentMethod'>) => Promise<PaymentResponse>;
  handleInitNuveiSimplyConnect: any;
}
