...
const usePurchase = () => {
      ...
      if (['Authorised', 'RedirectShopper', 'IdentifyShopper', 'ChallengeShopper'].includes(response.resultCode)) {
        ...
        switch (response.action.type) {
          ...
          case 'threeDS2':
            ...
            break;
          case 'nuveiSimplyConnect':
            handleInitNuveiSimplyConnect(response, orderNumber);
            break;
        }
      } else {
        toast.error(
  ...
  const purchase = useCallback(async () => {
    ...
    if (paymentData?.type === 'nuvei') {
      const result = await makeNuveiPayment({
        orderNumber,
        amountPlanned: { currencyCode: transaction.total.currencyCode, centAmount: transaction.total.centAmount },
      });
